        ...
        <div className="mt-3 grid gap-3">
          {models.map((model) => (
            <ModelStatusCard
              ...
              requiredBySources={requiredRoles?.includes(model.role) ?? false}
              sourceDegraded={degradedRoles?.includes(model.role) ?? false}
            />
          ))}
        </div>
      )}
...
function ModelStatusCard({
  ...
  return (
    <article className="rounded-md border border-neutral-200 bg-neutral-50 p-3">
      ...
      {requiredBySources ? (
        <div className="mt-2 flex items-start gap-2 rounded-md border border-sky-200 bg-sky-50 px-2 py-2 text-xs text-sky-800">
          <AlertCircle className="mt-0.5 size-3.5 shrink-0" aria-hidden="true" />
          <span>
            {sourceDegraded
              ? "Required by current sources. Feature degraded until this role is active."
              : "Required by current sources."}
          </span>
        </div>
      ) : null}
