...
import { Metric } from "./components/source-config/metric";
import { ModelStatusPanel } from "./components/source-config/model-status-panel";
import type { SourceDraft } from "./components/source-config/source-draft";
import { SourceDraftRow } from "./components/source-config/source-draft-row";
import { SourceStatusCard } from "./components/source-config/source-status-card";
import { SupportedSourceTypeRow } from "./components/source-config/supported-source-type-row";
...
export function SourceConfigurationPage({
  ...
  const canPreview = configuredSources.length > 0 && !previewPending;
  ...
  const requiredModelRoles = Array.from(
    new Set(preview?.sources.flatMap((entry) => entry.inventory?.required_model_roles ?? []) ?? []),
  );
  const degradedModelRoles = Array.from(
    new Set(preview?.sources.flatMap((entry) => entry.inventory?.degraded_model_roles ?? []) ?? []),
  );
    ...
    <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      <div className="flex min-w-0 flex-col gap-5">
        ...
        <section className="rounded-lg border border-neutral-300 bg-white p-4 shadow-sm">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {(preview?.sources.map((entry) => entry.source) ?? config.sources).map((source) => {
              const sourcePreview = preview?.sources.find(
                (entry) => entry.source.spec === source.spec || entry.source.id === source.id,
              );
              return (
                <SourceStatusCard
                  inventory={sourcePreview?.inventory ?? null}
      ...
      <aside className="flex h-fit flex-col gap-5">
        <ModelStatusPanel
          ...
          degradedRoles={degradedModelRoles}
          ...
          requiredRoles={requiredModelRoles}
        />
