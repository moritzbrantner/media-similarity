...
export const pngPixelBase64 =
  "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+/p9sAAAAASUVORK5CYII=";

export const pngPixelDataUrl = `data:image/png;base64,${pngPixelBase64}`;
...
export function makeJob(overrides: DeepPartial<JobSnapshot> = {}): JobSnapshot {
  return {
...
export function makeJobEvents(job: JobSnapshot = defaultCompletedIndexJob): JobEvent[] {
  return [
    {
      job_id: job.spec.id,
...
export function makeSourceConfigResponse(
  overrides: DeepPartial<SourceConfigResponse> = {},
): SourceConfigResponse {
  return {
    ...
    sources: (overrides.sources ?? defaultSourceConfigResponse.sources).map((source, index) =>
      completeSourceConfigSource(source, index),
    ),
...
function completeSourceConfigSource(
  source: DeepPartial<SourceConfigSource>,
  index: number,
): SourceConfigSource {
  const spec = source.spec ?? `/source-${index + 1}`;
  const kind = source.kind ?? (spec.includes("://") ? spec.split("://")[0] : "local");
  return {
    capabilities: {
    ...
    id: source.id ?? `src_v1_fixture_${index + 1}`,
    kind,
    normalized_uri: source.normalized_uri ?? spec,
    spec,
    status: source.status ?? "ready",
  } as SourceConfigSource;
}

export function makeWorkflowConfigResponse(
  overrides: DeepPartial<WorkflowConfigResponse> = {},
): WorkflowConfigResponse {
  return {
...
export function makeResult(overrides: DeepPartial<SearchResult> = {}): SearchResult {
  return {
    ...defaultResult,
    ...overrides,
    image: {
...
export function makeScene(overrides: DeepPartial<SearchSceneResponse> = {}): SearchSceneResponse {
  return {
    ...defaultScene,
    ...overrides,
    results: overrides.results ?? defaultScene.results,
  } as SearchSceneResponse;
}

export function makeSearchResponse(overrides: DeepPartial<SearchResponse> = {}): SearchResponse {
  return {
...
const defaultCompletedIndexJob: JobSnapshot = {
  artifacts: [],
...
const defaultSourceConfigResponse: SourceConfigResponse = {
...
const defaultImage: ImagePayload = {
  animated_thumbnail_url: null as string | null,
  artifacts: [],
  audio_analysis: null,
  duration_ms: null as number | null,
  faces: [] as FaceDetectionPayload[],
...
const defaultScene: SearchSceneResponse = {
