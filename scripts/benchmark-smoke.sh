...
target_dir="$root/backend/target/performance-smoke"
...

{
  ...
  if [[ -f backend/Cargo.lock ]]; then
    printf 'cargo_lock_sha256=%s\n' "$(sha256sum backend/Cargo.lock | cut -d' ' -f1)"
  else
...
run_benchmark() {
  ...
  CARGO_TARGET_DIR="$target_dir" \
    cargo bench --manifest-path backend/Cargo.toml --bench performance_smoke -- "$@" \
    2>&1 | tee "$artifact_dir/$log"
}

base_sha="${PERF_BASE_SHA:-}"
bench_path="backend/benches/performance_smoke.rs"
...
if [[ -n "$base_sha" ]] && git cat-file -e "$base_sha:$bench_path" 2>/dev/null; then
  ...
  (
    cd "$baseline_dir"
    CARGO_TARGET_DIR="$target_dir" \
      cargo bench --manifest-path backend/Cargo.toml --bench performance_smoke -- --save-baseline=pr-base
